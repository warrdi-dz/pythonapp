...
@app.route("/")
def home():
    return jsonify({"status": "OK", "message": "GARAGE PRO V12 - DOMINANT TOLE"})


# =========================
# COULEUR DOMINANTE DE LA TOLE
# =========================
def compute_dominant_tole(hsv_img, body_mask, gray_img):
    """
    Calcule la couleur DOMINANTE (la plus presente) de la tole de la voiture.
    On utilise un histogramme 3D HSV avec binning et on prend le pic (mode).
    Retourne (H_dom, S_dom, V_dom, texture_ref).
    """
    valid = hsv_img[body_mask > 0]
    if len(valid) < 200:
        return None, None

    # Quantification pour trouver le pic dominant
    H_bins, S_bins, V_bins = 18, 8, 8
    Hq = (valid[:, 0].astype(np.int32) * H_bins // 180).clip(0, H_bins - 1)
    Sq = (valid[:, 1].astype(np.int32) * S_bins // 256).clip(0, S_bins - 1)
    Vq = (valid[:, 2].astype(np.int32) * V_bins // 256).clip(0, V_bins - 1)
    keys = Hq * (S_bins * V_bins) + Sq * V_bins + Vq
    counts = np.bincount(keys, minlength=H_bins * S_bins * V_bins)
    top_key = int(np.argmax(counts))

    h_top = top_key // (S_bins * V_bins)
    s_top = (top_key // V_bins) % S_bins
    v_top = top_key % V_bins

    # Selection des pixels du bin dominant +- 1
    mask_dom = (
        (np.abs(Hq - h_top) <= 1) &
        (np.abs(Sq - s_top) <= 1) &
        (np.abs(Vq - v_top) <= 1)
    )
    dom_pixels = valid[mask_dom]
    if len(dom_pixels) < 100:
        dom_pixels = valid

    dom_H = float(np.median(dom_pixels[:, 0]))
    dom_S = float(np.median(dom_pixels[:, 1]))
    dom_V = float(np.median(dom_pixels[:, 2]))

    # Texture globale (ecart-type du Laplacien sur la tole)
    lap = cv2.Laplacian(gray_img, cv2.CV_64F, ksize=3)
    lap_valid = lap[body_mask > 0]
    texture_ref = float(np.std(lap_valid)) if len(lap_valid) > 0 else 0.0

    return (dom_H, dom_S, dom_V), texture_ref


def get_poly_stats(hsv_img, body_mask, gray_img, polygon):
    h, w = hsv_img.shape[:2]
    ...
    lap = cv2.Laplacian(gray_img, cv2.CV_64F, ksize=3)
    lap_zone = lap[combined > 0]
    tex = float(np.std(lap_zone)) if len(lap_zone) > 0 else 0.0
    stats = {
...
def detect_front_rear(lights):
    ...
    white_thr = max(400, int(band_area * 0.020))
    if red_tot >= red_thr:
        facing = "rear"; log.append(f"facing=REAR (rouge tot={red_tot} >= {red_thr})")
    elif white_tot >= white_thr and white_tot > red_tot * 3:
        facing = "front"; log.append(f"facing=FRONT (blanc tot={white_tot})")
    else:
        facing = "side"; log.append(f"facing=SIDE (R={red_tot} W={white_tot})")
    ...
    log.append(f"R G={rl} D={rr} | W G={wl} D={wr}")
...
def estimate_angle(lights, crop_w, crop_h, facing):
    ...
    if sym_ratio >= 8.0: angle = 10.0
    elif sym_ratio >= 4.0: angle = 10.0 + (8.0 - sym_ratio) / 4.0 * 30.0
    elif sym_ratio >= 2.0: angle = 40.0 + (4.0 - sym_ratio) / 2.0 * 25.0
    elif sym_ratio >= 1.3: angle = 65.0 + (2.0 - sym_ratio) / 0.7 * 20.0
    else: angle = 87.0
...
def build_zones(crop_w, crop_h, angle, rear_side, front_side, facing, lights):
    ...
    def zone(name, a, b):
        return {"name": name, "poly": make_poly(crop_w, crop_h, int(a*crop_w), int(b*crop_w), top_base, bot_base, persp, tilt_dir)}
    log_label = ""
    if angle <= 25:
        log_label = f"PROFIL ({angle}°) near={near_side} {'AR' if is_rear else 'AV'}"
        if near_side == "right":
            if is_rear:
                return [zone("Aile AV",0.00,0.20),zone("Porte AV",0.20,0.48),
                        zone("Porte AR",0.48,0.78),zone("Aile AR",0.78,1.00)], log_label
            return [zone("Aile AR",0.00,0.20),zone("Porte AR",0.20,0.48),
                    zone("Porte AV",0.48,0.78),zone("Aile AV",0.78,1.00)], log_label
        if is_rear:
            return [zone("Aile AR",0.00,0.22),zone("Porte AR",0.22,0.52),
                    zone("Porte AV",0.52,0.80),zone("Aile AV",0.80,1.00)], log_label
        return [zone("Aile AV",0.00,0.22),zone("Porte AV",0.22,0.52),
                zone("Porte AR",0.52,0.80),zone("Aile AR",0.80,1.00)], log_label
    elif angle <= 55:
        log_label = f"3/4 LEGER ({angle}°)"
        if near_side == "right":
            if is_rear:
                return [zone("Porte AV",0.00,0.22),zone("Porte AR",0.22,0.50),
                        zone("Aile AR",0.50,0.72),zone(pc_label,0.72,1.00)], log_label
            return [zone("Porte AR",0.00,0.22),zone("Porte AV",0.22,0.50),
                    zone("Aile AV",0.50,0.72),zone(pc_label,0.72,1.00)], log_label
        if is_rear:
            return [zone(pc_label,0.00,0.28),zone("Aile AR",0.28,0.50),
                    zone("Porte AR",0.50,0.78),zone("Porte AV",0.78,1.00)], log_label
        return [zone(pc_label,0.00,0.28),zone("Aile AV",0.28,0.50),
                zone("Porte AV",0.50,0.78),zone("Porte AR",0.78,1.00)], log_label
    elif angle <= 80:
        log_label = f"3/4 MARQUE ({angle}°)"
        if near_side == "right":
            return [zone("Aile AR" if is_rear else "Aile AV",0.42,0.68),
                    zone(panel,0.68,0.85),zone(pc_label,0.85,1.00)], log_label
        return [zone(pc_label,0.00,0.15),zone(panel,0.15,0.32),
                zone("Aile AR" if is_rear else "Aile AV",0.32,0.58)], log_label
    else:
        log_label = f"FACE/DOS ({angle}°)"
        if is_rear:
            return [zone("Aile AR G",0.00,0.20),zone(pc_label,0.20,0.55),
                    zone(panel,0.45,0.80),zone("Aile AR D",0.80,1.00)], log_label
        return [zone("Aile AV G",0.00,0.20),zone(pc_label,0.20,0.55),
                zone(panel,0.45,0.80),zone("Aile AV D",0.80,1.00)], log_label
...
def analyse():
    try:
        ...
        if yolo_result.get("make"):  car_info["make"]  = yolo_result.get("make",  car_info["make"])
        if yolo_result.get("model"): car_info["model"] = yolo_result.get("model", car_info["model"])
        ...
        zones, zone_decision = build_zones(crop_w, crop_h, angle, rear_side, front_side, facing, lights)
        fr_log.append(f"Zones: {zone_decision}")

        hsv_full  = cv2.cvtColor(car_crop, cv2.COLOR_BGR2HSV)
        gray_full = cv2.cvtColor(car_crop, cv2.COLOR_BGR2GRAY)

        # Masque TOLE strict : on EXCLUE vitres, pneus, ombres, reflets purs.
        mask_dark   = cv2.inRange(hsv_full, (0, 0, 0),   (180, 255, 45))
        mask_sky    = cv2.inRange(hsv_full, (0, 0, 215), (180, 18, 255))
        mask_glass  = cv2.inRange(hsv_full, (90, 10, 60), (130, 90, 200))  # vitres bleutees
        mask_body = cv2.bitwise_not(cv2.bitwise_or(cv2.bitwise_or(mask_dark, mask_sky), mask_glass))
        ...
        # ===== COULEUR DOMINANTE DE LA TOLE =====
        dom_color, tex_ref = compute_dominant_tole(hsv_full, mask_body, gray_full)
        if dom_color is None:
            return jsonify({"error": "No body pixels"}), 400
        dom_H, dom_S, dom_V = dom_color
        fr_log.append(f"DOMINANT TOLE: H={dom_H:.0f} S={dom_S:.0f} V={dom_V:.0f} tex={tex_ref:.1f}")
        ...
        header = f"{car_info['make']} {car_info['model']} | {'AR' if facing=='rear' else ('AV' if facing=='front' else 'COTE')} | {angle}° | DomH{int(dom_H)}S{int(dom_S)}V{int(dom_V)}"
        ...
        for idx, zone in enumerate(zones, start=1):
            ...
            zone_color, px_count, stats = get_poly_stats(hsv_full, mask_body, gray_full, poly_local)

            if zone_color is None:
                color_rect, label_score, verdict = (150,150,150), "N/A", "Non analysable"
                diff_h = diff_s = diff_v_signed = std_s = std_v = tex_zone = 0.0
                signe = "?"
            else:
                zone_H = float(zone_color[0])
                zone_S = float(zone_color[1])
                zone_V = float(zone_color[2])

                # Ecart H (circulaire) vs DOMINANT
                dh = abs(zone_H - dom_H); diff_h = min(dh, 180.0 - dh)
                # Ecart S et V SIGNES vs DOMINANT
                diff_s        = zone_S - dom_S
                diff_v_signed = zone_V - dom_V
                diff_v_abs    = abs(diff_v_signed)

                std_s    = stats["std_s"]
                std_v    = stats["std_v"]
                tex_zone = stats["texture"]
                tex_ratio = tex_zone / max(tex_ref, 1e-3)

                signe = "+" if diff_v_signed > 0 else ("-" if diff_v_signed < 0 else "=")

                # ===== VERDICT base sur la TOLE DOMINANTE =====
                # Logique:
                #   - zone trop CLAIRE ou trop FONCEE vs dominant   -> peinture
                #   - ecart H significatif (si couleur saturee)     -> peinture
                #   - difference de saturation marquee              -> indice
                ...
                if tex_ratio < 0.55 or tex_ratio > 1.8:
                    tex_penalty = 6.0
                elif tex_ratio < 0.7 or tex_ratio > 1.45:
                    tex_penalty = 3.0
                score += tex_penalty
                ...
                if h_reliable:
                    # voiture coloree
                    if   56>=score >= 5.0: verdict_state = "refaite"
                    elif score >= 40.0: verdict_state = "suspecte"
                ...
                label_score = (
                    f"H{int(diff_h)}/dS{int(diff_s):+d}/dV{signe}{int(diff_v_abs)}"
                    f"/T{tex_ratio:.2f}/Sc{int(score)}"
                )
            ...
            label_full = f"{idx}. {zone['name']}  {label_score}"
            ...
            results_zones.append({
                "idx": idx, "zone": zone["name"],
                "diff_h": round(diff_h, 1),
                "diff_s": round(diff_s, 1),
                "diff_v": round(diff_v_signed, 1),
                "std_s": round(std_s, 2), "std_v": round(std_v, 2),
                "texture": round(tex_zone, 2),
                "pixels": px_count, "verdict": verdict,
                "polygon": poly_global.tolist()
            })

        diffs       = [abs(z["diff_v"]) + z["diff_h"] for z in results_zones]
        final_score = min(int(np.mean(diffs)) if diffs else 0, 100)
        if   final_score < 10: result = "Peinture homogene (OK)"
        elif final_score < 25: result = "Legeres variations detectees"
        ...
        return jsonify({
            ...
            "dominant_tole_hsv": {
                "H": round(dom_H, 1), "S": round(dom_S, 1), "V": round(dom_V, 1)
            },
            "texture_ref":     round(tex_ref, 2),
