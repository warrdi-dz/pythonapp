...
# 4 méthodes + vote majoritaire
# =========================
def detect_car_orientation(car_crop):
    """
    Méthode 1 : feux arrière ROUGES
    Méthode 2 : phares avant BLANCS/JAUNES
    Méthode 3 : position pare-brise (zone sombre haute)
    Méthode 4 : texture Sobel (arrière = plus de détails)
    ...
    # Bandes gauche/droite pour analyser les feux
    band_w  = int(crop_w * 0.22)
    band_y1 = int(crop_h * 0.38)
    ...
    left_hsv  = cv2.cvtColor(left_zone,  cv2.COLOR_BGR2HSV)
    right_hsv = cv2.cvtColor(right_zone, cv2.COLOR_BGR2HSV)

    # -------------------------------------------
    # MÉTHODE 1 : feux arrière ROUGES
    # Rouge HSV : H=[0-10] ou [170-180], S>70, V>70
    # -------------------------------------------
    def count_red(hsv):
    ...
    if red_left > red_right * 1.4:
        votes_right += 2   # feux rouges à gauche = arrière gauche = avant droite
        log.append("M1: rouge gauche → avant DROITE (+2)")
    elif red_right > red_left * 1.4:
        votes_left += 2    # feux rouges à droite = arrière droite = avant gauche
        log.append("M1: rouge droite → avant GAUCHE (+2)")
    else:
        log.append("M1: rouge neutre (pas de vote)")

    # -------------------------------------------
    # MÉTHODE 2 : phares avant BLANCS/JAUNES
    # Les phares avant sont très lumineux et clairs
    # HSV : S<80, V>180 (blanc/jaune clair)
    # On cherche des zones très brillantes dans
    # la bande basse (40%-85% hauteur)
    # -------------------------------------------
    def count_headlight(hsv):
        # Blanc très lumineux = phare avant allumé ou verre de phare
        white = cv2.inRange(hsv, (0,  0,  180), (180, 80,  255))
        # Jaune/blanc chaud = phare halogène
        yellow = cv2.inRange(hsv, (15, 30, 180), (40,  200, 255))
    ...
    if light_left > light_right * 1.4:
        votes_left += 2    # phares blancs à gauche = avant gauche
        log.append("M2: phares gauche → avant GAUCHE (+2)")
    elif light_right > light_left * 1.4:
        votes_right += 2   # phares blancs à droite = avant droite
        log.append("M2: phares droite → avant DROITE (+2)")
    else:
        log.append("M2: phares neutre (pas de vote)")

    # -------------------------------------------
    # MÉTHODE 3 : position du pare-brise
    # Grande zone sombre inclinée dans la partie haute
    # Le pare-brise est toujours du côté AVANT
    # -------------------------------------------
    gray      = cv2.cvtColor(car_crop, cv2.COLOR_BGR2GRAY)
    top_band  = gray[int(crop_h * 0.08):int(crop_h * 0.52), :]
    dark_mask = (top_band < 75).astype(np.uint8)
    col_sums  = dark_mask.sum(axis=0).astype(float)
    col_sums  = np.convolve(col_sums, np.ones(25) / 25, mode='same')
    total     = col_sums.sum()

    if total > 0:
        center_x = float(np.sum(np.arange(len(col_sums)) * col_sums)) / total
        ratio    = center_x / crop_w

        if ratio < 0.40:
            votes_left += 1
            log.append(f"M3: pare-brise gauche ({ratio:.2f}) → avant GAUCHE (+1)")
        elif ratio > 0.60:
            votes_right += 1
            log.append(f"M3: pare-brise droite ({ratio:.2f}) → avant DROITE (+1)")
        else:
            # Centre ambigu : on compare les deux moitiés
            left_dark  = col_sums[:crop_w // 2].sum()
            right_dark = col_sums[crop_w // 2:].sum()
            if left_dark > right_dark * 1.4:
                votes_left += 1
                log.append("M3: plus sombre gauche → avant GAUCHE (+1)")
            elif right_dark > left_dark * 1.4:
                votes_right += 1
                log.append("M3: plus sombre droite → avant DROITE (+1)")
            else:
                log.append("M3: pare-brise neutre (pas de vote)")

    # -------------------------------------------
    # MÉTHODE 4 : texture Sobel
    # L'arrière a plus de détails (feux complexes,
    # plaque, pare-choc chargé) → gradient plus fort
    # -------------------------------------------
    ...
    if right_detail > left_detail * 1.25:
        votes_left += 1    # plus de détails à droite = arrière droite = avant gauche
        log.append(f"M4: texture droite ({right_detail:.1f}>{left_detail:.1f}) → avant GAUCHE (+1)")
    elif left_detail > right_detail * 1.25:
        votes_right += 1   # plus de détails à gauche = arrière gauche = avant droite
        log.append(f"M4: texture gauche ({left_detail:.1f}>{right_detail:.1f}) → avant DROITE (+1)")
    else:
        log.append("M4: texture neutre (pas de vote)")

    # -------------------------------------------
    # VOTE FINAL
    # -------------------------------------------
    orientation = "left" if votes_left >= votes_right else "right"
    log.append(f"VOTE FINAL: gauche={votes_left} droite={votes_right} → {orientation}")

    return orientation, log
...
def analyse():
    try:
        ...
        if mirror_box is not None:
            ...
            if mirror_color is not None:
                ...
                if m_diff >= 14 and m_diff < 26:
                    m_color   = (0, 0, 255)
                    m_verdict = "Retro: suspect!"
                elif m_diff < 14:
                    m_color   = (0, 165, 255)
                    m_verdict = "Retro: variation"
                else:
                    m_color   = (0, 210, 0)
                    m_verdict = "Retro: OK"
            else:
            ...
            cv2.rectangle(final_img, (abs_mx1, abs_my1),
                          (abs_mx2, abs_my2), (0, 255, 255), 3)

            overlay2 = final_img.copy()
            cv2.rectangle(overlay2, (abs_mx1, abs_my2 + 2),
                          (abs_mx2, abs_my2 + 22), (0, 0, 0), -1)
        ...
        cv2.putText(
            ...
            f"Retro: {'OUI' if mirror_box is not None else 'NON'}",
            (10, img_h - 10),
